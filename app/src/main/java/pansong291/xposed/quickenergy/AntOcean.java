                ...
                try {
                    ...
                    if ("SUCCESS".equals(jo.getString("resultCode"))) {
                        if (jo.getBoolean("opened")) {
                            ...
                            Log.recordLog("神奇海洋请先开启神奇海洋，并完成引导教程");
                        }
                    } else {
        ...
        try {
            for (int i = 0; i < bubbleVOList.length(); i++) {
                ...
                if ("AVAILABLE".equals(bubble.getString("collectStatus"))) {
                    ...
                    String s = AntForestRpcCall.collectEnergy("xingzou",userId, bubbleId);
                    JSONObject jo = new JSONObject(s);
                    if ("SUCCESS".equals(jo.getString("resultCode"))) {
        ...
        try {
            ...
            if ("SUCCESS".equals(jo.getString("resultCode"))) {
                ...
                if (isFinish && !StringUtil.isEmpty(dstChapterCode)) {
                    ...
                    jo = new JSONObject(s);
                    if ("SUCCESS".equals(jo.getString("resultCode"))) {
                        Log.forest("神奇海洋切换到[" + dstChapterName + "]系列");
                    } else {
                        Log.i(TAG, jo.getString("resultDesc"));
                    }
                }
